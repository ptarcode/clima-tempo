	...
	var callback = function(docHtml){
		...
		for (var i=0;i<5;i++) {
			previsions.push({ 
				...
				'wind'                        : { 'direction' : wind['br'][strip(docHtml('.box-prev-completa .velocidade-vento-prev-completa .underlinenone').eq(1).text())],
												  'velocity'  : strip(docHtml('.box-prev-completa .velocidade-vento-prev-completa .underlinenone').eq(2).text())},
				'probability_of_precipitation': { 'volume'     : strip(docHtml('.box-prev-completa .prob-chuva-prev-completa  .underlinenone').eq(1).text()),
												  'percentage' : strip(docHtml('.box-prev-completa .prob-chuva-prev-completa  .underlinenone').eq(2).text())},
